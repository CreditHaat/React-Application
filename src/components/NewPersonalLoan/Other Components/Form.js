            ...
            <section className="container banner">
                <div className="row py-md-5 px-md-5" style={{ display: "flex" }}>
                    <div className="col-md-6">
                        ...
                        <img src={formPageImage} className="figure-img img-fluid banner_img" alt="..." /> 
                        ...
                        <div className="row" style={{ display: "flex" }}>
                            <form onSubmit={handleSubmit}>
                                <div className="input-group mb-5">
                                    <input type="text" className={`form-control textBox ${errors.firstName ? 'is-invalid' : ''}`} placeholder="First Name  (पहला नाम)" aria-label="First Name" aria-describedby="first-name-icon" name="firstName" value={formData.firstName || ''} onChange={handleChange}  />
                                    <span className="input-group-text" id="first-name-icon"><i className="bi bi-person"></i></span>
                                    {errors.firstName && <div className="invalid-feedback">{errors.firstName}</div>}
                                </div>
                                <div className="input-group mb-5">
                                    <input type="text" className={`form-control textBox ${errors.lastName ? 'is-invalid' : ''}`} placeholder="Last Name  (उपनाम)" aria-label="Last Name" aria-describedby="last-name-icon" name="lastName" value={formData.lastName || ''} onChange={handleChange} />
                                    <span className="input-group-text" id="last-name-icon"><i className="bi bi-person"></i></span>
                                    {errors.lastName && <div className="invalid-feedback">{errors.lastName}</div>}
                                </div>
                                <div className="input-group mb-2">
                                    <input type="text" className={`form-control textBox ${errors.mobileNumber ? 'is-invalid' : ''}`} placeholder="Mobile Number  (मोबाइल नंबर)" aria-label="Mobile Number" aria-describedby="mobile-number-icon" name="mobileNumber" value={formData.mobileNumber || ''} onChange={handleChange} />
